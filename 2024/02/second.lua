...
-- if this fails, try again without the right num that failed this.
local function CheckTable(tbl)
    -- dir (bool). increasing = true, decreasing = false 
    local dir
    local save = true
    local index_right_num
...
for line_content in io.lines("input.txt") do
    ...
    local index
    save, index = CheckTable(tbl)
    if not save then
        save = false
        for i = 1, #tbl do
            local copy = {unpack(tbl)}
            table.remove(copy, i)
            if CheckTable(copy) then
                save = true
                break
            end
        end
